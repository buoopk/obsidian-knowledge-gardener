...
export default class KnowledgeGardenerPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.addCommand({
            ...
            callback: () => this.showDailyPrescription(),
        });
    ...
    async loadSettings() {
        // loadData() returns the full persisted blob (settings + history).
        ...
        if (IS_PRO) {
            ...
            // Free build: strip history key if present so it doesn't
            // leak into settings via Object.assign
            if (data != null && typeof data === "object") {
                delete data.prescriptionHistory;
            }
        }
